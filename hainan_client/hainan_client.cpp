...
void download_file(SOCKET clientSock, const char* address, const int port,
                   const int max_speed, const char* target, const char* local) {
    send(clientSock, target, strlen(target) * sizeof(char*), NULL);
    cout << "已向服务器请求文件:" << target << endl;

    const BOOL status = judge_file_exist(clientSock);
    if (!status) return;
    ...
    if (fp == NULL) {
        ...
        cout << "接收完成	" << target << "共" << file_size / 1023.999 << "KB"
             << endl;
        cout << "另存为	" << local << endl;
        fclose(fp);
    }
...
// 127.0.0.1 3366 200 cbr.jar cbr.jar
...
int main(int argc, const char** argv) {
    ...
    while (true) {
        ...
        download_file(clientSock, address, port, max_speed, target.c_str(),
                      local.c_str());
